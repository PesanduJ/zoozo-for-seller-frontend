...
const WeeklySalesTable = () => {
  ...
  const filterSalesBySellerId = (sales) => {
    if (sellerId) {
  ...
  const filteredSalesData = filterSalesBySellerId(salesData);

  const calculateTotalSellingPrice = () => {
    if (filteredSalesData.length > 0) {
      return filteredSalesData.reduce((totalSellingPrice, sale) => totalSellingPrice + sale.sellingPrice, 0);
    }
    return 0;
  };
