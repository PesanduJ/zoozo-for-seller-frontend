...
function useAPIData() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_HOST}/api/admin/allAdmins`, {
...
function AdminsTable() {
  ...
  const handleBanAdmin = async (nic) => {
    setBanLoading(true);
  
    try {
      const response = await banAdmin(nic);
  ...
  const handleRemoveAdmin = async (nic) => {
    setDeleteLoading(true);
  
    try {
      const response = await removeAdmin(nic);
                ...
                {currentItems.map((data) => {
                  return (
                    <Table.Row key={data.id}>
                      ...
                      <Table.Cell>
                        <button className="ui grey button" onClick={() => handleBanAdmin(data.nic)}>{banLoading ? (
                            <Dimmer active inverted>
                        ...
                        onClick={() => handleRemoveAdmin(data.nic)}>{deleteLoading ? (
                            <Dimmer active inverted>
